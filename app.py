...
def run_task(script_path):
    full_path = os.path.join("jobs", script_path)
...
def add_task():
    ...
    task_folder = os.path.join('jobs', task_name)
    ...
    if file and file.filename.endswith('.py'):
        ...
        # Copiar arquivo existente para a nova pasta (se desejar mover)
        source_path = os.path.join('jobs', command_input)
    ...
    task = {
        'id': task_id,
        'name': data['name'],
        'command': f"{task_name[:8]}/{script_name}",
        'enabled': True,
        'last_run': None,
        'last_output': '',
        'cron': {
